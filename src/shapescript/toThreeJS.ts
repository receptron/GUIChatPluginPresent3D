...
import { Evaluator, SymbolTable, Value, RGBA, MaterialValue, FunctionValue, isObjectValue, iterationValues, rgbaOf, valueToString, valuesEqual } from "./evaluator";
...
export class Converter {
  ...
  private warnTexture(value: Value): void {
    if (value === "") return;
    this.warn(`texture "${valueToString(value)}" is not supported — the shape is drawn with its colour instead`);
  }
  ...
  private convertCustomShape(node: CustomShapeNode): THREE.Object3D | null {
    ...
    try {
      return this.inScope(group, () => {
        ...
        if (name !== undefined) group.name = valueToString(this.evaluator.evaluate(name as Expression));
  ...
  private applyExplicitTransforms(object: THREE.Object3D, properties: ShapeProperties, scaleBySize = false): void {
    ...
    if (properties.name !== undefined) object.name = valueToString(this.evaluator.evaluate(properties.name));
  }
