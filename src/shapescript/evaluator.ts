...
/** A value as text, written out rather than left to the default stringification.
 *  The result is what `String(value)` produced before this function existed — a
 *  tuple joins on commas, and an object-shaped value (a range, a function, a
 *  material, a mesh) has no textual form and keeps JavaScript's own wording. It
 *  is spelled out because the conversion of a union that includes objects is
 *  exactly where "[object Object]" reaches a user by accident. */
...
const builtInFunctions: Record<string, (...args: Value[]) => Value> = {
  ...
  trim: (s: Value) => valueToString(s).trim(),
  split: (s: Value, separator: Value) => valueToString(s).split(valueToString(separator ?? "")),
...
function contains(needle: Value, haystack: Value): boolean {
  ...
  if (typeof haystack === "string") return haystack.includes(valueToString(needle));
...
export class Evaluator {
    ...
    switch (expr.type) {
      ...
      case "subscript": {
        ...
        if (typeof index !== "number" || !Number.isInteger(index) || index < -value.length || index >= value.length)
          throw new Error(`Subscript out of range: ${valueToString(index)}`);
  ...
  private evaluateMaterial(expr: MaterialExpr): MaterialValue {
    ...
    if (texture) material.texture = valueToString(this.evaluate(texture));
    return material;
  }
